...
export function getVisitor(path: NodePath) {
    ...
    function visitVariableStatement(path: NodePath<VariableDeclaration>): Statement | undefined {
        if (containsYield(path)) {
            ...
            const assignExps = declarations.map(transformInitializedVariable).filter(Boolean) as Expression[];
    ...
    function beginCatchBlock(path: NodePath<CatchClause>): void {
        ...
        emitAssignment(name, factory.callExpression(factory.memberExpression(factory.cloneNode(state), factory.identifier("sent")), []));
        emitNop();
    }
    ...
    function build() {
        ...
        return generatorHelper(true, factory.cloneNode(state), buildResult, programPath);
    }
        ...
        if (clauses) {
            return factory.blockStatement([
                factory.switchStatement(
                    factory.memberExpression(factory.cloneNode(state), factory.identifier('label')),
                    clauses
                )
            ]);
        }
        ...
        if (statements) {
            ...
            if (currentExceptionBlock) {
                ...
                statements.unshift(
                    factory.expressionStatement(
                        factory.callExpression(
                            factory.memberExpression(factory.memberExpression(factory.cloneNode(state), factory.identifier("trys")), factory.identifier("push")),
                            [
                                factory.arrayExpression([
